...
export default async function postDetailService(userId: string, postId: string) {
  ...
  try {
    ...
    tempDetailData.longtitude = [
      result[0]['srcLongitude'],
      result[0]['wayOneLongitude'],
      result[0]['wayTwoLongitude'],
      result[0]['destLongitude'],
    ];
    tempDetailData.latitude = [
      result[0]['srcLatitude'],
      result[0]['wayOneLatitude'],
      result[0]['wayTwoLatitude'],
      result[0]['destLatitude'],
    ];
    ...
    const themePromise = new Promise(async (resolve, reject) => {
      ...
      const themeRet = await db.sequelize.query(themeQuery, {
    ...
    const warningPromise = new Promise(async (resolve, reject) => {
      ...
      const warningRet = await db.sequelize.query(warningQuery, {
        replacements: { postId: postId },
        type: QueryTypes.SELECT,
      });
    ...
    await Promise.all([themePromise, warningPromise])
      .then(() => {
        postDetailData.push(tempDetailData);
      })
      .catch((err) => {
        throw err;
      });
