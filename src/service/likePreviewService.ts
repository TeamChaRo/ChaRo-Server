...
export async function likeLocalService(local: string, userId: string){
    ...
    const result =  await db.sequelize.query(localQuery,{ type: QueryTypes.SELECT, replacements:{userId:userId, region:regionName}, raw:true, nest : true});
...
export async function likeThemeService(theme: string, userId: string){
    ...
    const result = await db.sequelize.query(themeQuery,{ replacements:{userId:userId, theme:themeName},type: QueryTypes.SELECT, raw:true, nest:true });
